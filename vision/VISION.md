...
Change Log: v2 → v2.1
AI scope widened from "code generation" to "software artifacts" (code, tests, docs, ops)
Intended Reader: added explicit exclusions (AI researchers, tool operators)
Core Belief: "proven" → "demonstrates its value through application" (avoids scientific-verification framing)
Scope/Glossary: added Draft vs Canonical distinction to prevent a glossary bottleneck on exploration
Evidence First: added Secondary Sources rule (aid explanation, cannot replace primary attribution)
Conceptual Integrity: unchanged (reviewer flagged as ready for Constitution as-is)
Modularity: "independently understood" → "clear responsibility + transparent dependency boundary"
Success Criteria: added Handbook-level criterion (traceability of its own recommendations), forming a reader + handbook dual loop
