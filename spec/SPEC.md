...
**Worked example:** none yet — `knowledge/principles/deep-modules.md` has not been authored. Scheduling its authorship is Plan's responsibility (see Plan, Phase 1).
...
## Change Log: v2 → v3 (Correction)

**This is a Correction, not an Evolution** (Constitution C-410). Section 4.2's Principle Template referenced "Section 7 of this Spec" as containing a worked instance of the `deep-modules` Principle. Section 7 is Non-Goals — it never contained any such instance. This was a stale cross-reference from an earlier draft structure, not a deliberate claim. Fixed to state plainly that the worked example does not yet exist and that scheduling its authorship belongs to Plan. No other content changed.

Full revision history: see `spec/SPEC-CHANGELOG.md`.
Author(s):        John Ousterhout
Type:             book
Year:             2018
Edition/Version:  2nd edition
